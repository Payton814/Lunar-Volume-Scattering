...
plt.plot(t2, Ey_resample_ref)
plt.plot(t2, Eyavg)
plt.plot(t, Eyavg0, linestyle = '--')
#plt.plot(t2, Ey_resample_ref)
plt.show()

dfout = pd.DataFrame({'Time (ns)': t2, 'Ez (V/m)': Eyavg})
dfout.to_csv('CE3_2m_Efield.csv', index = False)
...
plt.plot(fm, 2*(np.array(dfmatlab['S21'][:1200])), label = 'Waveguide Model')
plt.plot(f[:len(f)//2], 20*np.log10(abs(fft[:len(f)//2])/abs(fftref[:len(f)//2])), label = 'Dipole Source')
#plt.plot(f[:len(f)//2], spline(f[:len(f)//2]) - 20*np.log10(abs(fftref[:len(f)//2])/np.max(abs(fft[:len(f)//2]))))
plt.xlim((0,1))
plt.ylim((-7.0, 3))
plt.xlabel('Frequency (GHz)', fontsize = 15)
plt.ylabel('Power attenuated (dB)', fontsize = 15)
plt.title('Attenuation from CE3 rocks: 2m thick', fontsize = 15, fontweight = 'bold')
plt.grid()
plt.legend()
plt.show()
